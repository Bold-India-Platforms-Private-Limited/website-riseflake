...
function DeadlineCountdown({ deadline }: { deadline: string }) {
  ...
  const sub = urgent ? 'text-rose-500' : moderate ? 'text-amber-500' : 'text-indigo-400'
...
function ShareButtons({ title, url }: { title: string; url: string }) {
  ...
  return (
    <div>
      <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400 mb-2">Share this job</p>
      <div className="flex gap-2">
        ...
        <button
          onClick={copyLink}
          title="Copy link"
          className={`flex items-center justify-center rounded-lg border px-3 py-2 transition-colors text-xs font-semibold ${
            copied
    ...
    <aside className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="divide-y divide-slate-100">
          ...
          {!salaryInfo && (
            <InfoRow icon={<Banknote className="h-3.5 w-3.5" />} label="Salary" value={job.is_salary_hidden ? 'Confidential' : 'Not disclosed'} />
          )}
        </div>
