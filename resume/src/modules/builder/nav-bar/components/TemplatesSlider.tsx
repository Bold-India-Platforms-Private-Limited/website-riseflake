...
export const TemplateSlider = () => {
  ...
  const onChangeTemplate = (templateId: string) => {
    useTemplates.getState().setTemplate(availableTemplate[templateId]);
  };

  return (
    <div className="w-full mt-4">
      <div className="h-[500px] overflow-y-auto pr-2 custom-scrollbar grid grid-cols-2 gap-4">
        {Object.keys(availableTemplate).map((templateKey, index) => {
          const template = availableTemplate[templateKey];
...
export const TemplateSlide = ({ isActive, id, name, thumbnail, onChangeTemplate, index }: any) => {
  return (
    <div
    ...
    >
      <div className="aspect-[1/1.4] relative overflow-hidden rounded-md shadow-sm">
        <Image src={withBasePath(thumbnail)} alt={name} fill className="object-cover" />

        {isActive && (
          <div className="absolute top-1 right-1 bg-white rounded-full shadow-md z-10">
            <Image src={withBasePath('/icons/selected-tick.svg')} alt="selected" width={20} height={20} />
          </div>
        )}
      </div>
      <p className="text-center text-[10px] uppercase tracking-wider mt-2 font-bold text-gray-400 line-clamp-1">
        <span className="text-resume-800 mr-1">{index}.</span> {name}
      </p>
    </div>
  );
};
